    ...
    <div className="w-80 bg-[#F7F7F8] border-l border-[#D1D5DB] h-screen overflow-y-auto flex-shrink-0">
      ...
      <div className="p-4 space-y-4">
        {newsItems.map((item, index) => (
          <div 
            key={index} 
            className="bg-white rounded-xl p-4 border border-[#D1D5DB] hover:shadow-md transition-shadow cursor-pointer"
          >
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center space-x-2">
                <span className={`px-2 py-1 rounded-full text-xs font-medium font-inter ${getCategoryColor(item.category)}`}>
                  {item.category}
                </span>
                {item.isBreaking && (
                  <span className="px-2 py-1 bg-red-100 text-red-800 rounded-full text-xs font-medium font-inter">
                    BREAKING
                  </span>
                )}
              </div>
              <ExternalLink className="w-4 h-4 text-[#2D2D2F] opacity-50" />
            </div>
            
            <h3 className="text-sm font-medium text-[#2D2D2F] mb-2 font-inter leading-tight">
              {item.title}
            </h3>
      ...
      <div className="p-4 border-t border-[#D1D5DB]">
        <button className="w-full text-center text-sm text-[#10A37F] font-medium font-inter hover:text-[#0d8a6b] transition-colors">
          View All News
        </button>
      </div>
    </div>
